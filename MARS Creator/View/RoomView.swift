...
struct RoomView: View {
    ...
    @State private var hasAlertRoomPosition: Bool = true
    @State private var hasAlertRoomMarkerWidth: Bool = false
        ...
        NavigationStack {
            VStack {
                ...
                RoomTabsView(
                    ...
                    floor: floor,
                    building: building,
                    hasAlertRoomPosition: $hasAlertRoomPosition,
                    hasAlertRoomMarkerWidth: $hasAlertRoomMarkerWidth,
                    onMarkerUpdated: {
                        checkRoomMarkers() 
                    }
                )
...
//                )
    ...
    func checkRoomPosition() {
        let result = doesMatrixExist(for: room.name, in: floor.associationMatrix)

        DispatchQueue.main.async {
            if let sceneObjectsCount = room.sceneObjects?.count, sceneObjectsCount != 0 {
                hasAlertRoomPosition = !result
            } else {
                hasAlertRoomPosition = false
            }
        }
    }

    func checkRoomMarkers() {
        
        print("CHECK:\(room.referenceMarkers.count != 0) ")
        let isMarkerListEmpty = room.referenceMarkers.count == 0
        
        
        if let sceneObjectsCount = room.sceneObjects?.count, sceneObjectsCount != 0 {
            hasAlertRoomMarkerWidth = isMarkerListEmpty
        } else {
            hasAlertRoomMarkerWidth = false
        }
