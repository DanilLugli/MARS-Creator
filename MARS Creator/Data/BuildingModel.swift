                    ...
                    do {
                        ...
                        if fileManager.fileExists(atPath: associationMatrixURL.path),
                           let loadedMatrix = loadRoomPositionFromJson(from: associationMatrixURL, for: floor) {
                            ...
                            floor._roomPosition = loadedMatrix


                            for room in floor.rooms {
                                if floor._roomPosition[room.name] == nil {
                                    print("Room '\(room.name)' is missing in the association matrix. Adding identity matrix.")

                                    floor._roomPosition[room.name] = RoomPositionMatrix(
                                        name: room.name,
                            ...
                            print("Failed to load RoomPositionMatrix from JSON file for floor \(floorURL.lastPathComponent)")
                        }
                    } catch {
                        print("Error loading rooms for floor \(floorURL.lastPathComponent): \(error)")
                    }
